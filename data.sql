CREATE OR REPLACE DATABASE airline_feature_store ;
...
def generate_tickets_sold(row):
    plane_models_info = [
        {'PLANE_MODEL': 'Boeing 737', 'SEATING_CAPACITY': 150, 'MAX_RANGE_KM': 5780, 'FUEL_EFFICIENCY_KM_PER_L': 3.5},
...
def run(session):
    data = [
        ...
        ("John F. Kennedy International Airport", "JEK"),
        ("Harry Reid International Airport", "LAS"),
        ("Orlando International Airport", "NCO"),
    ...
    for date in flights_df['SCHEDULED_DEPARTURE_UTC'].dt.date.unique():
        for airport_code in flights_df['AIRPORT_CODE'].unique():
            ...
            day_airport_flights = flights_df[(flights_df['SCHEDULED_DEPARTURE_UTC'].dt.date == date) & (flights_df['AIRPORT_CODE'] == airport_code)]
...
# Random plane models and carrier codes
 ...
 #weather_dataset_expanded['DATETIME"] - weather _dataset expanded["DATETIME*].dt.tz localize(*UTC*)
 # Adding a RAIN_NM column to the weather dataset with default value 0 (no rain)
    ...
    for idx in non_delayed_indices:
        ...
        weather_dataset_expanded.loc[matching_rows, 'RAIN MM'] = np.random.uniform(0.05, 2.5)
 ...
 # Iterating through the dataframe to update the previous 45 rows for cach row where RAIN_MM_H > 0
...
select * from us_flight_schedules;
select * from  airport_weather_station;
select * from plane_model_attributes